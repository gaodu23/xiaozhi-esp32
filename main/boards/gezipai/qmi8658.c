 ...
 * @brief 初始化QMI8658传感器
 */
bool qmi8658_init(qmi8658_handle_t *handle, i2c_master_bus_handle_t i2c_bus, uint8_t device_addr) {
    if (handle == NULL) {
...
bool qmi8658_set_accel_odr(qmi8658_handle_t *handle, uint8_t odr) {
    ...
    uint8_t ctrl2_val = (odr << 4) | handle->accel_range;
    esp_err_t ret = qmi8658_write_reg(handle, QMI8658_CTRL2, ctrl2_val);
    return (ret == ESP_OK);
}
...
bool qmi8658_set_gyro_odr(qmi8658_handle_t *handle, uint8_t odr) {
    ...
    uint8_t ctrl3_val = (odr << 4) | handle->gyro_range;
    esp_err_t ret = qmi8658_write_reg(handle, QMI8658_CTRL3, ctrl3_val);
    return (ret == ESP_OK);
}
...
/**
 * @brief 释放QMI8658传感器资源
 */
void qmi8658_deinit(qmi8658_handle_t *handle) {
...
static esp_err_t qmi8658_write_reg(qmi8658_handle_t *handle, uint8_t reg, uint8_t data) {
    ...
    return i2c_master_transmit(handle->dev_handle, write_buf, sizeof(write_buf), 1000);
}
 ...
 */
static esp_err_t qmi8658_read_reg(qmi8658_handle_t *handle, uint8_t reg, uint8_t *data, uint8_t len) {
    return i2c_master_transmit_receive(handle->dev_handle, &reg, 1, data, len, 1000);
}
